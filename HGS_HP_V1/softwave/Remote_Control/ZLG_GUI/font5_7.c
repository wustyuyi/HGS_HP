...
void GUI_PutNumber(unsigned char x,unsigned char y,float number,FONT type)
{
  ...
  {
    temp[0]='-';
		if(type==FONT5x7)	GUI_PutChar(x,y,temp[0]);
		else GUI_PutChar8_8(x,y,temp[0]);
    x+=6;
    number=-number;
  }
  data=(int)number;
  ...
  i++;
  temp[i]='\0';
	if(type==FONT5x7)	GUI_PutString(x,y,point);
	else GUI_PutString8_8(x,y,point);
}
