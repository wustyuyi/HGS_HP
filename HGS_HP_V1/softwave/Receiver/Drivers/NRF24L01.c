...
u8 NRF24L01_TxPacket(u8 *txbuf,u8 len)
{
  ...
  u16 i=0;	
	NRF24L01_CE_L;																						//片选
	NRF24L01_Write_Buf(WR_TX_PLOAD, txbuf, len);		//写数据到TX BUF  32个字节TX_PLOAD_WIDTH
	NRF24L01_CE_H;//启动发送
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	NRF24L01_Write_Reg(FLUSH_TX,0xff);
	NRF24L01_Write_Reg(FLUSH_RX,0xff);
	NRF24L01_Write_Reg(WRITE_REG_NRF + NRF24L01_CONFIG, 0x0f);   		 // IRQ收发完成中断开启,16位CRC,主接收
...
void NRF24L01_State_Check(unsigned char * Rx_Buf)
{
  u8 sta= NRF24L01_Read_Reg(STATUS);  											//读取状态寄存器的值
	...
	{
		nrf_rx_ok_cnt++;
		rx_fifo_len=NRF24L01_Read_Reg(R_RX_PL_WID);//读取缓存数据长度
    if(rx_fifo_len<33)
		{
...
{	
	...
	NRF24L01_Write_Buf(0xa8, tx_buf, len); 			 // 装载数据 WR_TX_PLOAD
	NRF24L01_CE_H;		 //置高CE
}
...
void rc_pair_run(void)
{
  ...
  rc_pair.tx_addr[2]=TX_ADDRESS[2];
	...
	NRF24L01_CE_L;
	NRF24L01_Write_Reg(WRITE_REG_NRF+RF_CH,rc_pair.chl[3]);//设置RF通道
