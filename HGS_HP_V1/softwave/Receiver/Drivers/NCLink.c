...
@函数名：NCLink_Send_CalRawdata(float gyro_x_raw,float gyro_y_raw,float gyro_z_raw,
											          ...
											          float mag_x_raw ,float mag_y_raw ,float mag_z_raw)
...
@日期：2020年01月17日
****************************************************************************************/
void NCLink_Send_CalRawdata(uint8_t gyro_auto_cal_flag,
											      ...
											      float mag_x_raw ,float mag_y_raw ,float mag_z_raw)
{
  ...
  nclink_databuf[_cnt++]=NCLink_Head[0];
  nclink_databuf[_cnt++]=NCLink_Head[1];
  nclink_databuf[_cnt++]=NCLINK_SEND_CAL_RAW;
  nclink_databuf[_cnt++]=0;
...
/***************************************************************************************
@函数名：NCLink_Send_CalParadata(float gyro_x_offset,float gyro_y_offset,float gyro_z_offset,
...
@日期：2020年01月17日
****************************************************************************************/
void NCLink_Send_CalParadata(float gyro_x_offset,float gyro_y_offset,float gyro_z_offset,
														 float acce_x_offset,float acce_y_offset,float acce_z_offset,
														 float acce_x_scale ,float acce_y_scale ,float acce_z_scale,
										         float mag_x_offset ,float mag_y_offset ,float mag_z_offset,
										         float pitch_offset ,float roll_offset)
{
  ...
  nclink_databuf[_cnt++]=NCLink_Head[0];
  nclink_databuf[_cnt++]=NCLink_Head[1];
  nclink_databuf[_cnt++]=NCLINK_SEND_CAL_PARA;
  nclink_databuf[_cnt++]=0;
...
//		 NCLink_Send_CalParadata(gyro_offset.x,gyro_offset.y,gyro_offset.z,
//												     B[0]*GRAVITY_MSS,B[1]*GRAVITY_MSS,B[2]*GRAVITY_MSS,
//												     K[0],K[1],K[2],
...
//    PPM_Databuf[7]=(int16_t)(*(data_buf+18)<<8)|*(data_buf+19);
  ...
  {		
    move_flag=*(data_buf+4),
		mode_order=*(data_buf+5);
    move_distance=(uint16_t)(*(data_buf+6)<<8)|*(data_buf+7);;
...
//			NCLink_Send_Obs_NE(Earth_Frame_To_XYZ.N,Earth_Frame_To_XYZ.E,GPS_Speed_Resolve[1],GPS_Speed_Resolve[0]);
		...
		{
			NCLink_Cnt=6;
//			NCLink_Send_Obs_UOP(WP_Sensor.baro_altitude,US_Distance,opt_gyro_data.y,opt_gyro_data.x);
		}
...
//														NamelessQuad.Speed[_PITCH]);	
		...
		{
			NCLink_Cnt=9;
			NCLink_Send_Userdata(RC_Data.rcdata[0],
													 RC_Data.rcdata[1],
													 RC_Data.rcdata[2],
													 RC_Data.rcdata[3],
													 RC_Data.rcdata[4],
		...
		{
			NCLink_Cnt=0;
//		  NCLink_Send_CalRawdata(Gyro_Safety_Calibration_Flag,
...
void NCLink_GS_Send_GPS_Guide_Flight(uint32_t lon,uint32_t lat)
{
  ...
  data_to_send[cnt++]=0x0C;
  data_to_send[cnt++]=0;

	data_to_send[cnt++]=BYTE3(lon);
	data_to_send[cnt++]=BYTE2(lon);
	data_to_send[cnt++]=BYTE1(lon);
	data_to_send[cnt++]=BYTE0(lon);
	data_to_send[cnt++]=BYTE3(lat);
	data_to_send[cnt++]=BYTE2(lat);
	data_to_send[cnt++]=BYTE1(lat);
	data_to_send[cnt++]=BYTE0(lat);
  ...
  data_to_send[cnt++]=NCLink_End[1];
	USART1_Send_Dma(data_to_send,15);
}
