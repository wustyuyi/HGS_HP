...
#define PPM_OUT_H  {GPIO_WriteBit(GPIOC,GPIO_Pin_13,Bit_SET)  ;GPIO_WriteBit(GPIOA,GPIO_Pin_1,Bit_SET);}	//输出高电平
#define PPM_OUT_L  {GPIO_WriteBit(GPIOC,GPIO_Pin_13,Bit_RESET);GPIO_WriteBit(GPIOA,GPIO_Pin_1,Bit_RESET);}//输出低电平
...
void TIM2_IRQHandler(void)
{
  ...
  if( TIM_GetITStatus(TIM2,TIM_IT_Update)!=RESET )
  {
    ...
    case 2://第一通道结束,开始0.5ms低电平---------------（2）
        PPM_OUT_L;
				toal_time+=500;
        TIM_SetAutoreload(TIM2,500*ppm_scale-1);
        ppm_count++;
        break;
    case 3://第二通道开始------------------------------（3）
        PPM_OUT_H;
